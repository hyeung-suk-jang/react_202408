...
const useCart = () => {
  ...
  const changeCount = (productId: string, mode: "increase" | "decrease") => {
    ...
    if (mode === "increase") {
      setCookies(COOKIE_KEY, encodeURIComponent(JSON.stringify([...productIds, productId])), {
        path: "/",
      });
    }
  };
  ...
  useEffect(() => {
    if (productIds && productIds.length) {
      ...
      const requestIds = productIds.reduce((acc, cur) => acc.set(cur, (acc.get(cur) || 0) + 1), new Map<string, number>());
