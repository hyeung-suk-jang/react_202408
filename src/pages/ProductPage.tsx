...
const ProductPage = () => {
  ...
  const { request: deleteProductRequest, loading: deleteProductLoading } = useAsync(() => deleteProduct(productId!), {
  ...
  return (
    <>
      <Container maxWidth="sm">
        <Box sx={{ display: "flex", justifyContent: "center", mb: 4 }}>{product?.thumbnail && <img src={`${API_SERVER_DOMAIN}/${product.thumbnail}`} alt={product?.name} style={{ width: "100%", maxWidth: 400 }} />}</Box>
        <Box
          sx={{
            display: "flex",
            alignItems: "center",
        ...
        >
          ...
          <ButtonGroup orientation="horizontal">
            ...
            <Button variant="text" onClick={handlePushPurchasePage} color="info">
              <Edit />
            </Button>
          </ButtonGroup>
        </Box>
      ...
      <Dialog open={isDeleteModal} onClose={() => setIsDeleteModal(false)} aria-labelledby="responsive-dialog-title">
        <DialogTitle id="responsive-dialog-title">상품을 정말로 삭제하시겠습니까?</DialogTitle>
        <DialogContent>
      ...
      <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)} aria-labelledby="responsive-dialog-title">
        <DialogTitle id="responsive-dialog-title">장바구니에 성공적으로 추가하였습니다.</DialogTitle>
        <DialogContent>
          <DialogContentText>장바구니 페이지로 이동하시겠습니까?</DialogContentText>
        </DialogContent>
        <DialogActions>
